...
| `geoserver` | GeoServer 3.0 | OGC services: WMS 1.3.0, WFS 2.0 (+CQL), OGC API Features **+ vector tiles (MVT)** | *(base)* |
...
2. WFS query of the tube zones **per genus separately** (budget: 800 ÷ number of genera; the polygons travel to GraphHopper as a POST body, nginx limit 16 MB). Important: a shared `IN (…)` limit favours the genus with the "first" table rows at the cap and silently swallows the others — observed live: birch+lime yielded exactly the birch-only route. The hits are bundled into **one MultiPolygon** (one custom-model area, one condition). **Convergence:** if avoidance pushes the route out of the tube seen so far, the zones along the *new* route are fetched, the sets merged and the route recomputed — until nothing new appears (max. 2 extra rounds). Only when optimisation and counting see the **same** zone set are the comparison numbers free of contradictions (before, "the detour" could cross more zones than the direct route). **Zone cache:** per leg (profile + start + end) the client keeps the baseline route and the converged zones, genera individually; factor changes then cost ~1.5 s instead of ~10 s (measured in Vienna), new genera only fetch their own delta. The cache is deliberately just a *seed* — the convergence verification always runs and feeds it back additively.
...
4. **Crossing statistics + direct-route comparison in the client:** GraphHopper does not report which areas a route crosses — so the client samples both lines in ~25 m steps (ray casting against the zone polygons) and shows "crosses N zones (X km in zones)" for the avoidance route (green at 0, per-genus tooltip) next to "M considered" — plus the **direct route** (pass 1 exists anyway) as a dashed comparison line with the same metrics. Vienna measurement (lime, 50×): direct 16.5 km with 6.3 km *inside* zones — avoidance route 26.1 km with 147 m inside zones.
...
A place inside the overlap of two city margins (Ruhr area!) deliberately lands in both files — every file is self-contained. **Deduplication happens in the photon entrypoint while merging** (jq splits batch lines carrying several places, awk lets only the first occurrence of each `place_id` through): Photon itself rejects duplicates (`op_type=create` → HTTP 409) and would otherwise abort the whole import.
...
| **`restart: unless-stopped` + a crashing one-time import = infinite loop** | The overlay counts the cities up **again and again**, "running for 3 min" after 12 hours, SSDs glowing all night (~18 GB written **per round**) | The Photon import (one single, long Java run) crashed (cause: duplicates, see next row — first wrongly blamed on the heap), the container exited, and `restart: unless-stopped` dutifully started over. A one-shot step inside an always-restart container **must** have a crash counter: after 3 failed attempts the entrypoint now halts honestly (`sleep infinity`), the status names attempt and remedy, and changed settings reset the counter automatically (the import heap has been tunable via `.env` since). Rule of thumb: "running for 3 minutes" after 12 hours is not a clock — it's a roundabout. |
| **"It already deduplicates" — asserted, never verified** | The 19-city import dies after 4.35 M places: `version_conflict_engine_exception … 2911 failed items in bulk` — while every city imports fine on its own | City margins overlap on purpose (every file is self-contained — a place may live in several). The docs claimed "Photon deduplicates via the place_id" — **an assumption written down as fact**: Photon imports with `op_type=create`, every duplicate is a 409, and the whole import blows up. It surfaced only on the first real multi-city import — single-city tests *cannot* find this bug, because only the overlap creates it. Fix: dedup while merging in the entrypoint (jq splits batch lines, awk: first occurrence per `place_id` wins). Rule of thumb: whatever the docs claim about third-party behaviour is fiction until a test confirms it — and overlap features need overlap tests. |
