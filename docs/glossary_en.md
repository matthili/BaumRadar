...
**MapLibre (GL JS / Native)** — Open-source rendering library for vector tiles, forked from Mapbox GL JS in December 2020 after Mapbox went proprietary. Renders on the GPU via WebGL (the browser's graphics-card interface): continuous zoom, rotation, 3D, globe; the appearance comes from a style specification. "GL JS" runs in the browser, "Native" on Android/iOS/desktop. No WFS, no GetFeatureInfo, essentially web Mercator only. Since the dual-renderer demo, MapLibre is the WebGIS's **second map engine** (toggle "Rendering: Browser (local)", beta): it reads vector tiles from the GeoServer tile cache and filters genera as pure style filters without a server round trip. OpenLayers remains the default — WFS+CQL, GetFeatureInfo and GPX are its home turf.
...
**Capabilities (GetCapabilities)** — An OGC service's self-description: *one* XML document answering "who runs me, which layers do I have, in which coordinate systems and formats, which operations can I perform, at which addresses". The counterpart of WSDL (SOAP) or OpenAPI/Swagger (REST) — or, more down to earth, of a command-line tool's `--help`, only standardised and machine-readable. Every client starts here; it is how the nine KRZN municipalities were discovered.
...
**INSPIRE** — EU directive (2007) for a European spatial data infrastructure: obliges public authorities to publish their geodata through harmonised models and **OGC-based services** (view: WMS/WMTS; download: WFS/Atom, increasingly OGC API Features). No obligation for BaumRadar as a private project — but the reason the OGC track is a given in the public sector. Mind the split: **service conformance** (mandatory entries in the [[Capabilities]], a GeoServer extension handles it in minutes) is the easy half; **data harmonisation** — mapping your holdings onto the INSPIRE data model of the relevant theme — is months of work. Particularly pertinent for a statistics office: "Statistical units" and "Population distribution" are INSPIRE themes in their own right, as is the grid system.

**Metadata (service vs. dataset)** — Information *about* data, in two clearly separate kinds. **Service metadata** describe the service itself (operator, operations, layers) and live in the capabilities document. **Dataset metadata** describe the holdings: spatial extent, survey date, update cycle, lineage, accuracy, responsible body, licence, keywords — standardised as ISO 19115 and discoverable through a catalogue service (CSW). BaumRadar's [`catalog.json`](data_structure_en.md) is exactly that in miniature: name, bounding box, download and signature URL plus version per city — everything you need to know *before* touching the actual data.
...
**Style specification (MapLibre/Mapbox style spec)** — JSON document that gives a vector-tile map its appearance: sources, layers, data-driven expressions — evaluated only in the client. The client-side counterpart of server-side SLD. Used here by the MapLibre engine (browser rendering); the server view keeps styling via SLD — visual changes now touch both worlds.

**Vector tiles / MVT (Mapbox Vector Tiles)** — Tiles containing **raw geometries + attributes** instead of finished images (compact Protobuf); rendering happens only in the browser (MapLibre), styled via a style specification. An open de-facto industry standard by Mapbox, but **not a ratified OGC standard** — OGC covers the tiling side through "OGC API – Tiles", which can deliver MVT. Benefits: tiny files, servable statically/via CDN (content delivery network), restyling without rebuilding a tile cache. In use since the dual-renderer demo: GeoServer emits MVT via the `vectortiles` extension, and the loader precomputes generalisation stages (city points → grid cells with dominant genus → individual trees) — one Vienna overview tile shrinks from ~4.5 MB raw to ~1.5 kB.

**WCS (Web Coverage Service)** — OGC standard for **raster data carrying measured values** instead of finished pictures: elevation models, satellite scenes, temperature grids. The difference to WMS is the whole point — WMS delivers an image to look at, WCS the numbers per cell to compute with. Not used here (BaumRadar only knows points and polygons); it appears because INSPIRE lists it next to WMS and WFS as the third service type.
