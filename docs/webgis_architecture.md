...
2. WFS-Query der Schlauch-Zonen **je Gattung einzeln** (Budget: 300 ÷ Anzahl Gattungen). Wichtig: Ein gemeinsames `IN (…)`-Limit bevorzugt am Deckel die Gattung mit den „ersten" Tabellenzeilen und verschluckt andere komplett — live beobachtet: Birke+Linde ergab exakt die Birken-Route. Die Treffer werden zu **einem MultiPolygon** gebündelt (eine Custom-Model-Area, eine Bedingung).
...
4. **Querungs-Statistik im Client:** GraphHopper meldet nicht, welche Areas eine Route durchquert — der Client tastet die finale Linie daher in ~25-m-Schritten ab (Ray-Casting gegen die Zonen-Polygone) und zeigt ehrlich „quert N Zonen" (grün bei 0, Tooltip mit Aufschlüsselung je Gattung) neben „M berücksichtigt" („300+" = Limit erreicht).
